...
class SfmModel_depth(nn.Module):
    """
    ...
    def compute_inv_depths2(self, image, disp):
        """Computes inverse depth maps from single images"""
        ...
        flip_lr = random.random() < self.flip_lr_prob if self.training else False
        inv_depths = make_list(flip_model(self.depth_net2, image, flip_lr))
        # If upsampling depth maps
        if self.upsample_depth_maps:
            inv_depths = interpolate_scales(
                inv_depths, mode='nearest', align_corners=None)
    ...
    def forward(self, batch, return_logs=False):
        ...
        # Generate inverse depth predictions
        # if self.training:
        #     prev_inv_depths = self.compute_inv_depths(batch['rgb_context'][0])
        # else :
        #     prev_inv_depths = self.compute_inv_depths(batch['rgb'])

        prev_inv_depths = self.compute_inv_depths(batch['rgb_context'][0])
        inv_depths = self.compute_inv_depths2(batch['rgb'],prev_inv_depths[0])
