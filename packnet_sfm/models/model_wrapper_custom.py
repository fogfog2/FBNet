...
class ModelWrapper(torch.nn.Module):
    ...
    def validation_step(self, batch, *args):
        ...
        output = self.evaluate_depth2(batch)
        if self.logger:
    ...
    def evaluate_depth(self, batch):
        ...
        # Get predicted depth
        inv_depths = self.model(batch)['prev_inv_depths']
        depth = inv2depth(inv_depths[0])
        # Post-process predicted depth
        batch['rgb'] = flip_lr(batch['rgb'])
        inv_depths_flipped = self.model(batch)['prev_inv_depths']
    ...
    def evaluate_depth2(self, batch):
        """Evaluate batch to produce depth metrics."""
        # Get predicted depth
...
def setup_depth_net2(config, prepared, **kwargs):
    ...
    depth_net = load_class_args_create(config.name,
        paths=['packnet_sfm.networks.depth2',],
        args={**config, **kwargs},
    )
    if not prepared and config.checkpoint_path is not '':
        depth_net = load_network(depth_net, config.checkpoint_path,
