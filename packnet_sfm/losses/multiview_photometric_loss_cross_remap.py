...
from packnet_sfm.geometry.camera_utils import view_synthesis, view_synthesis_with_depth , view_synthesis_with_disp
...
class MultiViewPhotometricLossCrossRemap(LossBase):
    """
...
########################################################################################################################
    ...
    def calc_depth_matching_loss(self, prev_disp, remap_disp, disp_loss_weight):
        l1_loss_remap = [torch.abs(prev_disp[i] - remap_disp[i])
                   for i in range(self.n)]

        depthmetric_loss = [l1_loss_remap[i].mean(1, True)
                                for i in range(self.n)]

        depthmetric_loss = sum([depthmetric_loss[i].mean()
                                for i in range(self.n)]) / self.n

        depthmetric_loss = depthmetric_loss * disp_loss_weight
...
########################################################################################################################
    ...
    def calc_smoothness_loss(self, inv_depths, images):
        ...
        smoothness_loss = 0.001 * smoothness_loss
...
########################################################################################################################
    ...
    def forward(self, image, context, inv_depths,
        ...
        back_photometric_losses = [[] for _ in range(self.n)]
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            #ref_warped, remap_depth = self.warp_ref_image_and_depth(inv_depths, ref_image, K, ref_K, pose)
            #remap_disp = depth2inv(remap_depth)
            ref_warped, remap_disp = self.warp_ref_image_and_disp(inv_depths, ref_image, K, ref_K, pose, prev_inv_depths)
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            back_ref_warped, back_remap_disp = self.warp_ref_image_and_disp(prev_inv_depths, image, K, ref_K, inverse_pose, inv_depths)

            # Calculate and store image loss
            ref_images = match_scales(ref_image, inv_depths, self.n)
            photometric_loss = self.calc_photometric_loss(back_ref_warped, ref_images)
            for i in range(self.n):
                back_photometric_losses[i].append(photometric_loss[i])
            # If using automask
            if self.automask_loss:
                # Calculate and store unwarped image loss
                unwarped_image_loss = self.calc_photometric_loss(ref_images, images)
                for i in range(self.n):
                    back_photometric_losses[i].append(unwarped_image_loss[i])
        # Calculate reduced photometric loss
        loss += self.reduce_photometric_loss(back_photometric_losses)


        #loss += self.calc_depth_matching_loss(remap_disp, prev_inv_depths, self.smooth_loss_weight)
        loss += self.calc_depth_matching_loss(remap_disp, inv_depths, self.smooth_loss_weight)
        loss += self.calc_depth_matching_loss(back_remap_disp, prev_inv_depths, self.smooth_loss_weight)

        # Include smoothness loss if requested
        #if self.smooth_loss_weight > 0.0:
        loss += self.calc_smoothness_loss(inv_depths, images)
        loss += self.calc_smoothness_loss(prev_inv_depths, ref_images)
